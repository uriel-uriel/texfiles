...
\begin{definition}[Distancia entre nodos]
...
\begin{figure}
  ...
  \begin{subfigure}{0.5\textwidth}
    \begin{tikzpicture}[scale=0.6, node/.style={circle, draw, fill=blue!20, radius=0.3cm}]
      \draw [black!20, thin] (-0.3,-0.3) grid (4.3,3.3);
      \node[node] (c) at (0,2) {c};
      \node[node] (e) at (4,1) {e};
      \node[node] (f) at (2,1) {f};
      \node (a) at (0,0) {...};
      \node (b) at (2,0) {...};
      \node (h) at (4,0) {...};
      \coordinate (cf) at (2,2) {};
      \coordinate (ce) at (4,2) {};
      \graph {
        (a) -- (c) -- (cf) -- {
          (f) -- {
            (e), (b)
    ...
    \end{tikzpicture}
    \caption{Este mapeo no cumple la tercera condici\'on.}
  \end{subfigure}
...
\begin{definition}[Notaci\'on $\mathcal{O}$]
